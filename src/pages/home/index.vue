...
//首页数据,首页最重要的是数据的获取以及数据的展示
//为什么要设置初始值，是因为，如果得到数据直接赋值，会丢失响应性
...
onMounted(async()=>{
    ...
    //如果不用then，一定不要缺少async和awit，要不然数据还未得到就赋值，是赋值不了的
...
const goOrderTwo=(index)=>{
    //根据医院索引值？找到具体医院对象，获取id?
    //但是这个索引不是表示“就医陪诊”or“陪护”吗，怎么能表示是哪个医院呢？
}
//医院列表点击
const goOrder=(data)=>{
